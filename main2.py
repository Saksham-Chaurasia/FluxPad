...
from app_profiles import NUMPAD_APPS, KEYBOARD_APPS
...
class App:
    def __init__(self):
        ...
        # --- Ghost Display Variables ---
        self.current_text = "" 
        self.cursor_on = True
        self.stop_threads = False
        self.display_visible = False 
        self.last_typing_time = 0
        self.height_animating = False 

        # --- CONTEXT AWARE VARIABLES (Loaded from file) ---
        self.last_active_window_title = ""
        self.numpad_apps = NUMPAD_APPS
        self.keyboard_apps = KEYBOARD_APPS
        ...
        self.root.after(200, lambda: self.toggle_input_view() if self.is_keyboard_view else self.build_numpad())
    ...
    def setup_ui(self):
        ...
        # 2. Bottom Bar (Resize Grip + Toggles)
        ...
        # --- FIX: PACK MEDIA AT TOP (So it doesn't move) ---
        self.media_frame = tk.Frame(content, bg=config.BG_COLOR)
        self.media_frame.pack(side="top", fill="x", pady=(0, 10))
    ...
    def check_app_context(self):
        ...
        if "FloatPad" in active_title or active_title == self.last_active_window_title:
            return

        self.last_active_window_title = active_title
        title_lower = active_title.lower()

        for app in self.numpad_apps:
            if app in title_lower:
                if self.is_keyboard_view:
                    self.root.after(0, self.toggle_input_view)
                return 

        for app in self.keyboard_apps:
            if app in title_lower:
                if not self.is_keyboard_view:
                    self.root.after(0, self.toggle_input_view)
                return 
    # -----------------------------------------

    def animate_height(self, start_h, end_h, callback=None):
        self.height_animating = True
        steps = 12 
        dt = 15     
        ...
        def _step(i):
            if i > steps:
                self.root.geometry(f"{w}x{end_h}")
                self.height_animating = False
            ...
            curr_h = int(start_h + (end_h - start_h) * ease)
            self.root.geometry(f"{w}x{curr_h}")
            self.root.update_idletasks()
            self.root.after(dt, lambda: _step(i + 1))
            
        _step(0)

    def show_typing_display(self):
        self.last_typing_time = time.time()
        
        if self.is_docked or self.display_visible: return 

        self.display_visible = True
        
        # Pack at BOTTOM now
        self.display_frame.pack(side="bottom", fill="x", pady=(8, 0))
        
        try:
            geo = self.root.geometry()
            h = int(geo.split('+')[0].split('x')[1])
            self.animate_height(h, h + 43)
        except: pass

    def hide_typing_display(self):
        if self.display_visible and not self.height_animating:
            self.display_visible = False
            try:
                geo = self.root.geometry()
                h = int(geo.split('+')[0].split('x')[1])
                target_h = max(100, h - 43)
                ...
                self.animate_height(h, target_h, on_finish)
            except: pass
    ...
    def toggle_input_view(self, event=None):
        ...
        if self.display_visible:
             try:
                 w, h = map(int, current_wh.split('x'))
                 current_wh = f"{w}x{h-43}" 
             except: pass
    ...
    def timer_loop(self):
        while not self.stop_threads:
            ...
            # --- AUTO-HIDE TYPING CHECK (Step 1 Logic) ---
            if self.display_visible and (time.time() - self.last_typing_time > 2.0):
                self.root.after(0, self.hide_typing_display)
            
            # --- STEP 2: CHECK CONTEXT ---
            self.root.after(0, self.check_app_context)
            # -----------------------------
            ...
            # --- FIX: DOCKING DELAY WHILE TYPING ---
            # If recently typed (< 15s), pause docking
            time_since_typing = time.time() - self.last_typing_time
            if time_since_typing < 15: 
                time_since_interaction = 0 # Hack: trick the logic to think we just interacted
            # ---------------------------------------
    ...
    def update_ghost_dock(self, x, y, w, h):
        if self.ghost_window is None:
            ...
            self.ghost_window.configure(bg="#222222") # Dark Grey (Visible)
            self.ghost_window.attributes("-alpha", 0.6) # More Opaque
    ...
    def stop_move(self, e):
        ...
        # Callback when magnet animation finishes
        def on_magnet_finish(mode):
            # Calculate the exact Dock Target based on the Snap Mode
            # We use the MONITOR edges, not the current window position, to be precise.
            
            dock_x = self.root.winfo_x()
            dock_y = self.root.winfo_y()
            
            if mode == 'left':
                dock_x = mon['l']
            elif mode == 'right':
                dock_x = mon['r'] # Pass the Right Wall coordinate
            elif mode == 'top':
                dock_y = mon['t']
            
            # Force dock at the calculated edge
            self.dock_window(animate=True, force_mode=mode, force_x=dock_x, force_y=dock_y)

        # Check Magnet
        snapped, mode = check_magnet_snap(self.root, mon, 150, on_magnet_finish)
        
        if not snapped:
            if self.is_docked:
                self.last_dock_geo = self.root.geometry()
                self.save_config()
            else:
                self.save_config()
    ...
    def setup_context_menu(self):
        ...
        def toggle_startup(): set_startup(self.run_startup_var.get())

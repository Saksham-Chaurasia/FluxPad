...
def apply_acrylic(hwnd, use_glass=True, opacity_value=242):
    try:
        policy = ACCENT_POLICY()
        policy.AccentState = 3 if use_glass else 0
        
        # Combine the opacity (0-255) with the color (0x1e1e1e)
        # We shift opacity_value 24 bits to the left to put it in the Alpha slot
        gradient_color = (opacity_value << 24) | 0x1e1e1e
        
        policy.GradientColor = gradient_color if use_glass else 0xFF1e1e1e
...
class App:
    def __init__(self):
        ...
        self.glass_opacity = self.settings.get("glass_opacity", 242)
    ...
    def refresh_visuals(self):
        try:
            hwnd = ctypes.windll.user32.GetParent(self.root.winfo_id())
            # Pass the stored opacity variable here
            apply_acrylic(hwnd, self.use_glass, self.glass_opacity)
        ...
        try:
            def load_and_process(path, size=(22, 22)):
                ...
                if getattr(sys, 'frozen', False):
                    # If running as EXE, look in the same folder as the .exe file
                    base_path = os.path.dirname(sys.executable)
                else:
                    # If running as script (VS Code), look in the script's folder
                    base_path = os.path.dirname(os.path.abspath(__file__))
                
                # 2. Join the base path with the requested image path (e.g., "icon/play.png")
                full_path = os.path.join(base_path, path)

                # 3. Check and Load
                if not os.path.exists(full_path):
                    # Print error (invisible in --noconsole mode, but good for debugging)
    ...
    def setup_context_menu(self):
        ...
        # --- NEW: Opacity Menu ---
        self.opacity_menu = tk.Menu(self.context_menu, tearoff=0, bg=BG_COLOR, fg=TXT_COLOR)
        self.opacity_menu.add_command(label="High (Solid-ish)", command=lambda: self.set_opacity(242))
        self.opacity_menu.add_command(label="Medium", command=lambda: self.set_opacity(180))
        self.opacity_menu.add_command(label="Low (Very Glassy)", command=lambda: self.set_opacity(100))
        self.opacity_menu.add_command(label="Ghost (Almost Invisible)", command=lambda: self.set_opacity(50))
        # -------------------------
        ...
        self.context_menu.add_cascade(label="Glass Opacity", menu=self.opacity_menu)
